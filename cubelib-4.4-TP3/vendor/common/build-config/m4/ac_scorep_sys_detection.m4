...
AS_IF([test "x${ac_scorep_cross_compiling}" = "x"],
    [AS_CASE([${ac_scorep_platform}],
         ...
         [cygwin],  [ac_scorep_cross_compiling="no"],
...
[
    ...
    AM_CONDITIONAL([PLATFORM_CYGWIN],  [test "x${ac_scorep_platform}" = "xcygwin"])
    ...
    AM_COND_IF([PLATFORM_CYGWIN],
        [AC_DEFINE([HAVE_PLATFORM_CYGWIN], [1], [Set if we are building for the Cygwin platform])])
