...
#ifndef __CUBE_CANONIZER_H
#define __CUBE_CANONIZER_H
...
Canonizer::run( const char* in_name, const char* out_name, std::ostream* name_list )
{
    ...
    if ( services::check_file( in_name ) != 0 )
    {
        throw Error( std::string( "File " ) + in_name = " failed check!" );
    }
...
/*#ifndef CUBE_COMPRESSED
   ifstream in(in_name);
   #else
   gzifstream in(in_name, ios_base::in|ios_base::binary);
   #endif
   if (!in)
    throw Error(string("Could not create stream for input file ") + in_name + "!");
...
/*
   ...
   #else
   gzofstream out(out_name, ios_base::out|ios_base::binary);
   #endif
   if (!out)
    throw Error(string("Could not create stream for output file ") + in_name +"!");
