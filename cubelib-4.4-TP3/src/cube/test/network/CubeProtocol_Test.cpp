...
TEST_F( CubeProtocol, throwsNetworkErrorIfSetToUnsupportedVersion ) {
    ASSERT_THROW( protocol.setVersion( protocol.getMaxVersion() + 1 ), cube::NetworkError);
}
...
TEST_F( CubeProtocol, throwsNetworkErrorOnCreatingUnregisteredRequest ) {
    ASSERT_THROW( protocol.createRequest( NUM_NETWORK_REQUESTS, 0 ), cube::NetworkError );
}
