                                  ...
                                  )
{
    ...
    else
    {
        ...
        for ( size_t i = 0; i < ntid; ++i )
        {
            ...
            ( *_tmp_value ) = ( __tmp_locs != NULL ) ? __tmp_locs[ i ] : 0.;
            _locs[ i ]      = _tmp_value;
        }
    ...
    if ( !isCustom() )
    {
        ...
        for ( size_t i = 0; i < lgv.size(); ++i )
        {
            ...
            for ( size_t j = 0; j < lg->num_children(); j++ )
            {
                ...
                ( *( inclusive_values[ lg->get_sys_id() ] ) ) += _locs[ _loc->get_id() ];
                ...
                {
                    ( *( inclusive_values[ _lg_parent->get_sys_id() ] ) ) += _locs[ _loc->get_id() ];
    ...
    else
    {
        ...
        {
            LocationGroup* lg = lgv[ i ];
            ( *( inclusive_values[ lg->get_sys_id() ] ) ) = this->get_sev( cnode, cnf, lg, cube::CUBE_CALCULATE_INCLUSIVE );
        }
        for ( size_t i = 0; i < stnv.size(); ++i )
        {
            SystemTreeNode* stn = stnv[ i ];
            ( *( inclusive_values[ stn->get_sys_id() ] ) ) = this->get_sev( cnode, cnf, stn, cube::CUBE_CALCULATE_INCLUSIVE );
        }
    }
    return;
}
