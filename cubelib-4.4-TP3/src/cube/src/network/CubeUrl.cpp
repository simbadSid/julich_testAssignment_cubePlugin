...
{
    // short-hand definition for iterator type
    ...
    iterator_t   token_it = search(url_string.begin(),
                                   url_string.end(),
    ...
    if (mProtocol != "file")
    {
        ...
        if (*it == ':')
        {
            ...
            advance(it, 1);
            iterator_t port_end = find_first_of(it,
                                                url_string.end(),
                                                port_and_path_delimiter.begin(),
                                                port_and_path_delimiter.end());
    ...
    iterator_t::difference_type len = distance(it, url_string.end());
    if (len != 0)
    {
...
Url::toString() const
{
    ...
    else
    {
        ...
        if (!(getPort() == this->getDefaultPort()))
        {
            portStr << ":" << getPort();
        }
