...
//         cubepl2_memory_manager->put( cubepl2::CUBE_STN_DESCRIPTION, _id, _stn->get_description(), CUBEPL_RESERVED_VARIABLE   );
        ...
        cubepl2_memory_manager->put( cubepl2::CUBE_STN_PARENT_ID, _id, ( _stn->get_parent() != NULL ) ? _stn->get_parent()->get_id() : -1., ( uint32_t )-1, CUBEPL_RESERVED_VARIABLE  );
...
void
CubePL2MemoryInitializer::memory_setup( cube::Metric* metric, uint32_t met_id  )
{
    cubepl2_memory_manager->put( cubepl2::CALCULATION_METRIC_ID, 0, metric->get_id(), met_id, CUBEPL_VARIABLE  );
}


void
CubePL2MemoryInitializer:: memory_setup( cube::Cnode* cnode, uint32_t met_id  )
{
    cubepl2_memory_manager->put( cubepl2::CALCULATION_CALLPATH_ID, 0,  cnode->get_id(), met_id, CUBEPL_VARIABLE  );
}


void
CubePL2MemoryInitializer:: memory_setup( cube::Region* region, uint32_t met_id  )
{
    cubepl2_memory_manager->put( cubepl2::CALCULATION_REGION_ID, 0,  region->get_id(), met_id, CUBEPL_VARIABLE  );
}

void
CubePL2MemoryInitializer::memory_setup(  cube::Sysres* sysres, uint32_t met_id  )
{
    cubepl2_memory_manager->put( cubepl2::CALCULATION_SYSRES_ID,   0, sysres->get_id(), met_id, CUBEPL_VARIABLE   );
    cubepl2_memory_manager->put( cubepl2::CALCULATION_SYSRES_KIND,   0, sysres->get_kind(), met_id, CUBEPL_VARIABLE  );
}
